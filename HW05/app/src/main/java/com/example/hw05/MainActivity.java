...
public class MainActivity extends AppCompatActivity implements LoginFragment.ILogin, RegisterFragment.IRegister, ForumsFragment.IForum, NewForumFragment.INewForum {
    ...
    @Override
    public void navigateToForum(Forum forum, Map<String, String> userMap) {
        getSupportFragmentManager()
                .beginTransaction()
                .replace(R.id.containerView, ForumFragment.newInstance(forum, userMap), NEW_FORUM_FRAGMENT_TAG)
                .addToBackStack(null)
                .commit();
    }
